...
impl CustomPathDef {
    ...
    /// parse: (in custom::path as custom_ident)
    /// 
    /// See [`NamedTypePathDef::parse`]
    ...
    /// parse: in custom::path as custom_ident
    /// 
    /// See [`CustomPathDef::parse_parenthesized`]
...
impl Parse for NamedTypePathDef {
    /// parse: (in custom::path as custom_ident) origin_path
    fn parse(input: ParseStream) -> syn::Result<Self> {
        // Example: (in custom::path as custom_ident) origin_path
        ...
        // Usually, the original path must start with `::`, which is the complete type path.
        //
        // Specifically, for primitive types such as `u64`, it is not necessary to start with `::`,
        // but they must not have a custom name.
